...
def caja_historico():
    try:
        ...
        for t in tickets:
            ...
            if dk not in dias: dias[dk]={'ventas':0,'tickets':0,'premios':0,'pendientes':0}
            ...
            elif p>0: dias[dk]['pendientes']+=1
        resumen=[]
        for dk in sorted(dias.keys()):
            d=dias[dk]; cd=d['ventas']*com_pct
            resumen.append({'fecha':dk,'tickets':d['tickets'],'ventas':round(d['ventas'],2),
                            'premios':round(d['premios'],2),'comision':round(cd,2),
                            'balance':round(d['ventas']-d['premios']-cd,2),'pendientes':d['pendientes']})
        tc=tv*com_pct
        return jsonify({'resumen_por_dia':resumen,
                        'totales':{'ventas':round(tv,2),'premios':round(tp,2),
                                   'comision':round(tc,2),'balance':round(tv-tp-tc,2)}})
    except Exception as e:
        return jsonify({'error':str(e)}),500
    ...
    try:
        ...
        for ag in ags:
            ...
            data.append({'nombre':ag['nombre_agencia'],'usuario':ag['usuario'],
                         ...
                         'premios_pendientes':round(pend,2),'comision':round(com,2),
                         'balance':round(ventas-pp-com,2),'tickets':len(mts)})
            tv+=ventas; tp+=pp; tc+=com
        return jsonify({'agencias':data,'global':{'ventas':round(tv,2),'pagos':round(tp,2),
                        'comisiones':round(tc,2),'balance':round(tv-tp-tc,2)}})
    except Exception as e:
        return jsonify({'error':str(e)}),500
...
def reporte_agencias_rango():
    try:
        ...
        for t in all_t:
            ...
            if t['pagado']: stats[aid]['premios_pagados']+=p
            elif p>0: stats[aid]['premios_pendientes']+=p
        out=[]
    ...
    try:
        ...
        return jsonify({'resumen_por_dia':resumen,
                        'totales':{'ventas':round(total_v,2),'premios':round(total_p,2),
                                   'comisiones':round(tc,2),'balance':round(total_v-total_p-tc,2),
                                   'tickets':total_t}})
    except Exception as e:
        return jsonify({'error':str(e)}),500
    ...
    try:
        ...
        for sel,monto in sorted(apuestas.items(),key=lambda x:x[1],reverse=True):
            ...
            riesgo_d[f"{sel} - {ANIMALES.get(sel,sel)}"]={
                'apostado':round(monto,2),'pagaria':round(monto*mult,2),
                'es_lechuza':sel=="40",
...
def tripletas_hoy():
    try:
        ...
        for tr in trips:
            ...
            gano=len(salidos)==3
            if gano: ganadoras+=1
            out.append({'id':tr['id'],'serial':tr['serial'],
                        ...
                        'monto':tr['monto'],'premio':tr['monto']*PAGO_TRIPLETA if gano else 0,
                        'gano':gano,'salieron':salidos,'pagado':bool(tr['pagado'])})
        return jsonify({'tripletas':out,'total':len(out),'ganadoras':ganadoras,
    ...
    try:
        ...
        for s in sorted(stats.values(),key=lambda x:x['ventas'],reverse=True):
            ...
            w.writerow([s['nombre'],s['usuario'],s['tickets'],
                        round(s['ventas'],2),round(s['premios'],2),round(com,2),round(s['ventas']-s['premios']-com,2)])
            tv+=s['ventas']
...
<div class="info">ZOOLO CASINO Local v1.0</div>
</div></body></html>'''
...
<div class="main">
  ...
  <div id="panel-tripleta" style="display:none">
    <div class="section-title">🎯 Tripleta <small style="color:#666">(x60 - Todo el día)</small></div>
  ...
  <div class="carrito-section">
    ...
    <div id="carrito-contenido"><div class="carrito-empty">Ticket vacío</div></div>
...
<div class="modal-content">
  <div class="modal-header"><h3>📊 Resultados</h3><button class="btn-close" onclick="closeModal('modal-resultados')">✕</button></div>
...
<div class="modal-content">
  <div class="modal-header"><h3>🎫 Mis Tickets</h3><button class="btn-close" onclick="closeModal('modal-mis-tickets')">✕</button></div>
...
<div class="modal-content">
  <div class="modal-header"><h3>💰 Caja</h3><button class="btn-close" onclick="closeModal('modal-caja')">✕</button></div>
  <div id="caja-hoy"></div>
  <hr style="border-color:#333;margin:15px 0">
  <h4 style="color:#ffd700;margin-bottom:10px">📅 Histórico</h4>
  <div class="filter-row">
    <input type="date" id="caja-ini">
    <input type="date" id="caja-fin">
  </div>
  <button class="btn-consultar" onclick="consultarCajaHist()">VER HISTÓRICO</button>
  <div id="caja-hist"></div>
</div></div>

<div class="modal" id="modal-anular">
<div class="modal-content">
  <div class="modal-header"><h3>❌ Anular Ticket</h3><button class="btn-close" onclick="closeModal('modal-anular')">✕</button></div>
...
<div class="modal-content">
  <div class="modal-header"><h3>🔎 Buscar Ticket</h3><button class="btn-close" onclick="closeModal('modal-buscar')">✕</button></div>
  <label style="color:#aaa">Serial:</label>
...
function agregarAlCarrito(){
  ...
  if(tipoActual==='tripleta'){
    ...
    carrito.push({tipo:'tripleta',hora:'TODO EL DIA',seleccion:tripSeleccionados.join(','),monto,
    ...
    carrito.push({tipo:'especial',hora:horaActual,seleccion:selEspActual,monto,desc:selEspActual+' x2'});
  } else {
    if(!selAnimal){toast('Seleccione un animal','error');return;}
    if(!horaActual){toast('Seleccione un horario','error');return;}
    carrito.push({tipo:'animal',hora:horaActual,seleccion:selAnimal,monto,desc:selAnimal+' - '+ANIMALES[selAnimal]});
  }
...
function renderCarrito(){
  ...
  if(carrito.length===0){c.innerHTML='<div class="carrito-empty">Ticket vacío</div>';tot.style.display='none';return;}
  let html='',total=0;
...
async function vender(){
  if(carrito.length===0){toast('Ticket vacío','error');return;}
  ...
  fetch('/api/caja').then(r=>r.json()).then(d=>{
    ...
    document.getElementById('caja-hoy').innerHTML=`
      ...
      <div class="caja-grid">
        ...
        <div class="caja-card"><div class="caja-label">COMISIÓN</div><div class="caja-val">S/${d.comision.toFixed(2)}</div></div>
        <div class="caja-card"><div class="caja-label">BALANCE</div><div class="caja-val ${col}">S/${d.balance.toFixed(2)}</div></div>
      </div>
      ${d.tickets_pendientes>0?`<div style="background:rgba(243,156,18,.1);border:1px solid #f39c12;border-radius:8px;padding:10px;text-align:center;color:#f39c12">⚠️ ${d.tickets_pendientes} ticket(s) ganador(es) sin cobrar</div>`:''}`;
  });
}
...
function consultarCajaHist(){
  ...
  .then(r=>r.json()).then(d=>{
    ...
    html+=`</div><div class="stats-box" style="margin-top:10px">
      ...
      <div class="stat-row"><span class="stat-label">Comisión</span><span class="stat-value">S/${d.totales.comision.toFixed(2)}</span></div>
  ...
  <div class="form-box">
    ...
    <div id="rep-resumen" style="display:none">
      <div class="stats-grid" style="margin-top:15px">
        ...
        <div class="stat-card"><h3>COMISIÓN</h3><p id="rep-c">--</p></div>
        <div class="stat-card"><h3>BALANCE</h3><p id="rep-b">--</p></div>
      </div>
      <h4 style="color:#ffd700;margin:15px 0 10px">Detalle por día</h4>
...
<div id="tab-agencias" class="tab-content">
  <div class="form-box">
    ...
    <div class="form-row">
      ...
      <input type="password" id="ag-password" placeholder="Contraseña">
  ...
  <div class="form-box">
    ...
    <div style="overflow-x:auto"><table>
      <thead><tr><th>ID</th><th>Usuario</th><th>Nombre</th><th>Comisión</th><th>Estado</th><th>Acciones</th></tr></thead>
...
function showTab(id){
  ...
  if(id==='resultados'){ let h=new Date().toISOString().split('T')[0]; document.getElementById('res-admin-fecha').value=h; document.getElementById('res-fecha-input').value=h; cargarResultadosAdmin(); }
  if(id==='tripletas') cargarTripletas();
  if(id==='riesgo') cargarRiesgo();
  if(id==='agencias') cargarAgencias();
}

function showMsg(id,msg,tipo){ let el=document.getElementById(id); el.innerHTML='<div class="msg '+tipo+'">'+msg+'</div>'; setTimeout(()=>el.innerHTML='',4000); }
function showGlobal(msg,tipo){ let el=document.getElementById('global-msg'); el.innerHTML='<div class="msg '+tipo+'" style="background:#111;padding:12px 20px;border-radius:10px;box-shadow:0 4px 20px rgba(0,0,0,.5)">'+msg+'</div>'; el.style.display='block'; setTimeout(()=>el.style.display='none',4000); }
    ...
    HORARIOS.forEach(h=>{
      ...
      html+=`<div class="resultado-item ${res?'tiene':''}">
        ...
        <div style="text-align:right;display:flex;flex-direction:column;align-items:flex-end;gap:4px">
          ...
          <button class="btn-editar" onclick="preSelRes('${h}','${f}',${res?`'${res.animal}'`:'null'})">${res?'✏️ Editar':'➕ Cargar'}</button>
        </div>
      </div>`;
    });
    c.innerHTML=html;
...
function guardarResultado(){
  ...
  }).catch(()=>showMsg('res-msg','Error de conexión','err'));
}
  ...
  fetch('/admin/riesgo').then(r=>r.json()).then(d=>{
    ...
    for(let[k,v] of Object.entries(d.riesgo)){
      html+=`<div class="riesgo-item ${v.es_lechuza?'lechuza':''}">
        <b>${k}${v.es_lechuza?' ⚠️ ALTO RIESGO':''}</b><br>
        <span style="color:#aaa;font-size:.82rem">Apostado: S/${v.apostado.toFixed(2)} | Pagaría: S/${v.pagaria.toFixed(2)} | ${v.porcentaje}%</span>
      </div>`;
    }
    l.innerHTML=html;
  });
  ...
  fetch('/admin/lista-agencias').then(r=>r.json()).then(d=>{
    ...
    d.forEach(a=>{
      t.innerHTML+=`<tr>
        ...
        <td><button class="btn-secondary" onclick="toggleAgencia(${a.id},${a.activa})">${a.activa?'Desactivar':'Activar'}</button></td>
      </tr>`;
    });
  });
}
...
function toggleAgencia(id,activa){
  fetch('/admin/editar-agencia',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({id,activa:!activa})}).then(r=>r.json()).then(d=>{
...
if __name__ == '__main__':
    ...
    print("="*50)
    print("  🦁 ZOOLO CASINO LOCAL v1.0")
    print("="*50)
    print(f"  Base de datos: {DB_PATH}")
    print(f"  URL: http://localhost:{port}")
    print(f"  Admin: usuario=admin  password=admin123")
    print("="*50)
    print("  Para ngrok: ngrok http "+str(port))
    print("="*50)
